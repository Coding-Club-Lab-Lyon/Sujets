...
# ========= IA (optimisée + anti demi-tour) =========

class NeonAI:
    """
    IA 'safe-first + territory + lookahead' avec anti demi-tour:
      - BFS torique (moi vs adversaires) → Voronoï exact.
      - Pénalités goulots/dead-ends, risque head-on (L/S/R ennemis).
      - Beam search borné (budget temps).
      - **Anti demi-tour** : bloque virtuellement la dernière case et
        interdit de revenir dessus, même si la grille n’est pas à jour.
    """
    # Réglages
    MAX_LOOKAHEAD_DEPTH = 8
    BEAM_WIDTH = 12
    TIME_BUDGET_SEC = 0.004
    AREA_CAP = 10_000
    ...
    def __init__(self):
        ...
        self.my_id = 0
        self.last_dir = RIGHT
        # Hints fournis par le client à chaque tick
        self.hint_prev_pos = None   # (x,y) dernière case (virtuellement bloquée)
        self.hint_cur_pos  = None   # (x,y) case actuelle
    ...
    def _blocked_mask(self, grid, W, H):
        ...
        # Blocage virtuel anti demi-tour: la case précédente de "moi"
        if self.hint_prev_pos is not None:
            px, py = self.hint_prev_pos
            blk[py*W + px] = True
        return blk
    ...
    def _evaluate_after_move(self, me, t, adverses, grid, W, H, blk=None, deg=None):
        ...
        # Anti demi-tour immédiat: interdire de revenir sur la dernière case
        if self.hint_prev_pos is not None and (nx, ny) == self.hint_prev_pos:
            return -1e12, None, nd
    ...
    def _plan(self, state):
        ...
        for t0 in (-1, 0, 1):
            ...
            roots.append({"score": s0, "turn0": t0, "pos": pos0, "dir": d0, "blk": blk})
        ...
        depth = 1
        while depth < self.MAX_LOOKAHEAD_DEPTH and (time.time() - start_t) < self.TIME_BUDGET_SEC:
            nxt = []
            for node in beam:
                if node["pos"] is None:
                    continue
                ...
                vdir = node["dir"]
                vblk = list(node["blk"])
                vblk[vy*W + vx] = True  # on bloque virtuellement la case occupée
...
class Client:
    ...
    def _handle_welcome(self, pkt):
        ...
        self.ai.set_welcome({"auto_kill": self.auto_kill})
    ...
    def _think_and_act(self, state):
        ...
        # Armer les hints pour l'IA (anti demi-tour local)
        self.ai.hint_cur_pos  = cur_pos
        self.ai.hint_prev_pos = self.last_pos
...
def main():
    ...
    ap.add_argument("--name", default="DiamondHead")
