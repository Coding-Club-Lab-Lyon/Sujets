...
# Directions & turns (doivent matcher l'énum C++)
...
class TronClient:
    ...
    def _send(self, t, payload=b"", ver=1):
        ...
        try:
            ...
            # Le serveur a probablement coupé
            pass
    ...
    def _send_input(self, turn: int, tick_hint: int = 0):
        # Clamp du turn dans [-1, 1] et envoi via _send
    ...
    def _handle_state(self, pkt: bytes):
        ...
        self.tick = tick
        self.W, self.H = W, H
        # Conserver la grille sous forme de liste d'octets (0 = vide, sinon id joueur)
        # list(...) -> ints Python
    ...
    def get_me(self):
        print(next((p for p in self.players if p["id"] == self.my_id), None))
