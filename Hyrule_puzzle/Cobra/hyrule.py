...
def hyrule_solver(board: Board) -> bool:
    ...
    for i in range(9):
        # Go to every column
        for j in range(9):
            # If the cell is empty try a number
            if board[i][j] == 0:
                # Try every possible value (1 - 9)
                for v in range(1, 10):
                    # If the value is ok then place it
                    if is_valid(board, i, j, v):
                        # Place it temporarly
                        board[i][j] = v
                        # If the next iteration is good then we say it is good
                        if hyrule_solver(board):
                            return True
                        # Else we back off
                        board[i][j] = 0
                # If nothing is good then is a fail
                return False
    # If no empty cell -> We won
    return True
